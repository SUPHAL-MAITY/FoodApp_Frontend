...
const AllCategories = () => {
  ...
  const handleEdit = (id, title, url) => {
    ...
    console.log(title);
    console.log(url);
    navigate("/admin/dashboard/update-categories", {
      state: { id, title, url },
    });
  };
  ...
  const handleDelete = (id) => {
    const { data } = axios.delete(
      `${import.meta.env.VITE_API}/api/category/delete/${id}`
    );
    if (data) {
          ...
          {categories?.map((c, i) => (
            <tbody key={i}>
              <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                <th
                  scope="row"
                  className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white font-serif"
                >
                  {c.title}
                </th>
                <td className="px-4 py-4">
                  <button
                    onClick={() => handleEdit(c._id, c.title, c.imageUrl)}
